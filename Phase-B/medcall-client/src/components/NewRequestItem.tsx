  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-base font-semibold text-secondary500">
          {location ? location : "temp_location"}
        </h3>
        <span className="text-base font-bold text-secondary500">
          {distance}
        </span>
      </div>
      <div className="flex gap-2">
        {status && (
          <span className="text-base px-3 py-2 rounded-full shadow-sm bg-[#f2e9e9] text-secondary500">
            {statusLabels[status as "starting" | "active" | "complete"]}
          </span>
        )}
        <span className="text-base px-3 py-2 rounded-full bg-secondary500 text-white">
          {typeOfEmergency}
        </span>
